// componentes/configuracionProyecto.js — Configuración del Proyecto y LocalStorage
...
export async function cargarConfiguracionProyecto() {
  try {
    ...
    const guardadoLocal = JSON.parse(localStorage.getItem('lpm_project_config') || '{}');
    ...
    for (const [clave, esquema] of Object.entries(PROJECT_FIELDS_SCHEMA)) {
      ...
      if (esquema.free) {
        ...
        if (valorBruto === 'localProcess_Manager' || valorBruto === 'Proyecto') {
          valorBruto = '';
        }
...
export async function guardarConfiguracionProyecto() {
  ...
  localStorage.setItem('lpm_project_config', JSON.stringify(datos));
  ...
  try {
    ...
    if (respuesta.ok) {
      showToast('Configuración del Proyecto guardada (LocalStorage & SQLite)');
    } else {
